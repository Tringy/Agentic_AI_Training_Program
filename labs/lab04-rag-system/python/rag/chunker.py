...
class CodeChunker:
    ...
    def _chunk_javascript(self, content: str, filename: str, language: str) -> List[CodeChunk]:
        ...
        if matches:
            ...
            return chunks + self._chunk_generic(content[first_match_start:], filename, language)
        else:
            return self._chunk_generic(content, filename, language)
    ...
    def _chunk_generic(self, content: str, filename: str, language: str) -> List[CodeChunk]:
        ...
        if current_chunk:
            ...
            chunks.append(
                CodeChunk(
                    content=chunk_content,
                    metadata={"filename": filename, "language": language, "type": "block", "line_start": chunk_start},
                    chunk_id=f"{filename}:lines_{chunk_start}",
                )
            )
    ...
    def _detect_language(self, filename: str) -> str:
        ...
        # also match filenames like "Dockerfile" (no extension)
        if lower.endswith("dockerfile") or lower == "dockerfile":
            return "dockerfile"
